    ...
    <div class="space-y-6">
      ...
      <ui-panel heading="registerRenderToolCall — server-side tool, browser renders">
        <p class="text-sm text-[var(--ink-soft)]">
          The DeepAgents graph owns the <code>getWeather</code> tool; the browser only
      ...
      <ui-callout tone="warn" title="The renderer name must equal the tool name">
        ...
        agent's tool by exact string. The DeepAgents graph in
        <code>backend/agent.ts</code> has to declare its tool as
      ...
      >
        <p class="mb-3 text-sm text-[var(--ink-soft)]">
          ...
          and no matching tool in <code>backend/agent.ts</code>: the frontend
