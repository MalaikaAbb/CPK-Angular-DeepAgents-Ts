...
interface Probe {
  ...
  detail: string;
  /**
   * Treat any HTTP response as "reachable", not just a 2xx. The DeepAgents
   * graph is served by the LangGraph API, which answers a probing GET with
   * whatever status that route carries — any of them still proves the process
   * is listening.
   */
  anyStatus?: boolean;
}
...
export class BackendHealth {
  ...
  protected readonly probes = signal<Probe[]>([
    ...
    {
      label: 'DeepAgents graph',
      url: 'http://localhost:2024/ok',
      hint: 'Start it with: npx @langchain/langgraph-cli dev (from backend/)',
      ok: null,
      detail: '',
      anyStatus: true,
    },
